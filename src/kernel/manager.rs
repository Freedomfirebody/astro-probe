...
impl WorkspaceManager {
    ...
    fn load_db_pool(&self, project_path: &str) -> anyhow::Result<DbPool> {
        ...
        let pool = crate::db::establish_connection_pool(&db_path)?;
        let conn = pool.get()?;
    ...
    pub fn create_workspace(&self, name: String, project_path: String) -> anyhow::Result<Workspace> {
        ...
        // Initialize db schemas and parse java files
        {
            ...
            let parser = crate::parser::JavaParser::new();
            ...
            if let Err(e) = crate::analyzer::jar::analyze_and_cache(&resolved_path, &conn, &id) {
                tracing::error!("Failed to analyze and cache JAR files: {}", e);
                return Err(anyhow::anyhow!("Failed to analyze and cache JAR files: {}", e));
            }
            if let Err(e) = crate::analyzer::DependencyInjectionAnalyzer::new().analyze(&conn) {
            ...
            if let Err(e) = crate::analyzer::CallGraphAnalyzer::new().analyze(&conn) {
                tracing::error!("Failed to run call graph analysis: {}", e);
                return Err(anyhow::anyhow!("Failed to run call graph analysis: {}", e));
            }
            if let Err(e) = crate::analyzer::DfgAnalyzer::new().analyze(&conn) {
    ...
    pub fn delete_workspace(&self, id: &str) -> bool {
        ...
        if let Some(state) = state {
            ...
            if let Ok(global_conn) = crate::db::establish_connection(crate::analyzer::jar::get_global_cache_path()) {
