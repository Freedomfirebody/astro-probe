...
fn resolve_method_ref(class_file: &cafebabe::ClassFile, cp_index: u16) -> Option<(String, String, String)> {
    let entry = class_file.constant_pool.get(cp_index as usize)?;
    match &**entry {
        cafebabe::constant_pool::ConstantPoolEntry::MethodRef(class_ref_cell, name_type_ref_cell) |
        cafebabe::constant_pool::ConstantPoolEntry::InterfaceMethodRef(class_ref_cell, name_type_ref_cell) => {
            let class_borrow = class_ref_cell.borrow();
            let name_type_borrow = name_type_ref_cell.borrow();
            
            // Get class name
            let class_name = if let cafebabe::constant_pool::ConstantPoolRef::Resolved(class_entry) = &*class_borrow {
                match &**class_entry {
                    cafebabe::constant_pool::ConstantPoolEntry::ClassInfo(utf8_ref_cell) => {
                        let utf8_borrow = utf8_ref_cell.borrow();
                        if let cafebabe::constant_pool::ConstantPoolRef::Resolved(utf8_entry) = &*utf8_borrow {
                            match &**utf8_entry {
                                cafebabe::constant_pool::ConstantPoolEntry::Utf8(s) => Some(s.replace('/', ".")),
                                _ => None,
                            }
                        } else {
                            None
            ...
            // Get name and type
            let (method_name, method_desc) = if let cafebabe::constant_pool::ConstantPoolRef::Resolved(nt_entry) = &*name_type_borrow {
                match &**nt_entry {
                    cafebabe::constant_pool::ConstantPoolEntry::NameAndType(n_cell, d_cell) => {
                        let n_borrow = n_cell.borrow();
                        let d_borrow = d_cell.borrow();
                        if let (cafebabe::constant_pool::ConstantPoolRef::Resolved(ne), cafebabe::constant_pool::ConstantPoolRef::Resolved(de)) = (&*n_borrow, &*d_borrow) {
                            match (&**ne, &**de) {
                                (cafebabe::constant_pool::ConstantPoolEntry::Utf8(n), cafebabe::constant_pool::ConstantPoolEntry::Utf8(d)) => {
...
fn resolve_class_name(class_file: &cafebabe::ClassFile, cp_index: u16) -> Option<String> {
    let entry = class_file.constant_pool.get(cp_index as usize)?;
    match &**entry {
        cafebabe::constant_pool::ConstantPoolEntry::ClassInfo(ref_cell) => {
            let cp_ref = ref_cell.borrow();
            if let cafebabe::constant_pool::ConstantPoolRef::Resolved(ref utf8_entry) = *cp_ref {
                match &**utf8_entry {
                    cafebabe::constant_pool::ConstantPoolEntry::Utf8(s) => Some(s.replace('/', ".")),
                    _ => None,
                }
...
fn resolve_name_and_type(class_file: &cafebabe::ClassFile, cp_index: u16) -> Option<(String, String)> {
    let entry = class_file.constant_pool.get(cp_index as usize)?;
    match &**entry {
        cafebabe::constant_pool::ConstantPoolEntry::NameAndType(name_ref_cell, desc_ref_cell) => {
            let name_borrow = name_ref_cell.borrow();
            let desc_borrow = desc_ref_cell.borrow();
            if let (cafebabe::constant_pool::ConstantPoolRef::Resolved(name_entry), cafebabe::constant_pool::ConstantPoolRef::Resolved(desc_entry)) = (&*name_borrow, &*desc_borrow) {
                match (&**name_entry, &**desc_entry) {
                    (cafebabe::constant_pool::ConstantPoolEntry::Utf8(name), cafebabe::constant_pool::ConstantPoolEntry::Utf8(desc)) => {
        ...
        for i in 0..archive.len() {
            ...
            if file.name().ends_with(".class") {
                ...
                if let Ok(class_file) = cafebabe::parse_class(&bytes) {
                    ...
                    let kind = if class_file.access_flags.contains(cafebabe::ClassAccessFlags::INTERFACE) {
                        "interface"
                    } else {
                        "class"
                    };
                    ...
                    for method in &class_file.methods {
                        ...
                        let param_names: Vec<String> = (0..param_types.len()).map(|idx| format!("p{}", idx)).collect();
                        ...
                        for attr in &method.attributes {
                            if let cafebabe::attributes::AttributeData::Code(ref code_attr) = attr.data {
                                let mut locals = vec![None; (code_attr.max_locals as usize).max(param_names.len() * 2 + 2)];
                                let is_static = method.access_flags.contains(cafebabe::MethodAccessFlags::STATIC);
                                    ...
                                    match opcode {
                                        25 => { // aload
                                            if ip + 1 < code.len() {
                                                ...
                                                let val = if idx < locals.len() { locals[idx] } else { None };
                                                stack.push(val);
                                                ip += 2;
                                            } else {
                                                ip += 1;
                                        ...
                                        42..=45 => { // aload_0 .. aload_3
                                            let idx = (opcode - 42) as usize;
                                            let val = if idx < locals.len() { locals[idx] } else { None };
                                            stack.push(val);
                                            ip += 1;
                                        }
                                        75..=78 => { // astore_0 .. astore_3
                                            let idx = (opcode - 75) as usize;
                                            let val = stack.pop().flatten();
                                            if idx < locals.len() {
                                                locals[idx] = val;
                                            }
                                        ...
                                        182..=185 => { // invokes
                                            if ip + 2 < code.len() {
                                                let cp_idx = u16::from_be_bytes([code[ip + 1], code[ip + 2]]);
                                                let extra = if opcode == 185 { 2 } else { 0 };
                                                ip += 3 + extra;
                                                
                                                if let Some((callee_class, callee_name, callee_desc)) = resolve_method_ref(&class_file, cp_idx) {
                                                    let callee_param_types = parse_descriptor(&callee_desc);
                                                    ...
                                                    let call_id = format!("{}:call_{}", method_fqn, call_site_counter);
                                                    call_site_counter += 1;
                                                    
                                                    let receiver_node = receiver_val.map(|param_idx| {
                                                        let param_name = if param_idx < param_names.len() {
                                                    ...
                                                    for (arg_idx, arg_val) in args.iter().enumerate() {
                                                        let arg_type = if arg_idx < callee_param_types.len() {
                                                        ...
                                                        let arg_var = match arg_val {
                                                            ...
                                                            None => format!("{}:call_arg_{}", call_id, arg_idx),
                                                        };
                                                        arg_vars.push(arg_var);
                                                    }
                                                    
                                                    let static_callee = format!("{}.{}({})", callee_class, callee_name, callee_param_types.join(","));
                                                    ...
                                                    for (arg_idx, (arg_var, arg_type)) in arg_vars.iter().zip(arg_types.iter()).enumerate() {
                                                        global_conn.execute(
                                                    ...
                                                    let callee_stripped = format!("{}.{}", callee_class, callee_name);
                                                    global_conn.execute(
                                        ...
                                        196 => {
                                             if ip + 1 < code.len() {
                                                 ...
                                                 match sub_op {
                                                     25 => {
                                                         if ip + 3 < code.len() {
                                                             let idx = u16::from_be_bytes([code[ip + 2], code[ip + 3]]) as usize;
                                                             let val = if idx < locals.len() { locals[idx] } else { None };
                                                             stack.push(val);
                                                             ip += 4;
                                                         } else {
                                           ...
                                           170 => {
                                               ...
                                               if ip + 12 <= code.len() {
                                                   let low = i32::from_be_bytes([code[ip+4], code[ip+5], code[ip+6], code[ip+7]]);
                                                   let high = i32::from_be_bytes([code[ip+8], code[ip+9], code[ip+10], code[ip+11]]);
                                                   ip += 12;
                                                   if high >= low {
                                                       if let Some(count) = high.checked_sub(low).and_then(|diff| diff.checked_add(1)) {
                                          ...
                                          171 => {
                                              ...
                                              if ip + 8 <= code.len() {
                                                  let npairs = i32::from_be_bytes([code[ip+4], code[ip+5], code[ip+6], code[ip+7]]);
                                                  ip += 8;
                                                  if npairs > 0 {
...
pub fn copy_jar_facts_to_local(conn: &Connection, jar_hash: &str) -> Result<()> {
    ...
    conn.execute(&format!("ATTACH DATABASE '{}' AS global_db", escaped_path), [])?;
...
fn is_db_locked_error(err: &anyhow::Error) -> bool {
    if let Some(sqlite_err) = err.downcast_ref::<rusqlite::Error>() {
        match sqlite_err {
            rusqlite::Error::SqliteFailure(ffi_err, _) => {
                if ffi_err.code == rusqlite::ErrorCode::DatabaseBusy || ffi_err.code == rusqlite::ErrorCode::DatabaseLocked {
                    return true;
                }
            }
            _ => {}
    ...
    err_str.contains("database is locked") 
        || err_str.contains("database file is locked") 
        || err_str.contains("error code 5") 
        || err_str.contains("error code 6")
}

pub fn analyze_and_cache(project_path: &str, conn: &Connection, workspace_id: &str) -> Result<()> {
    let res = (|| -> Result<()> {
        let global_db_path = get_global_cache_path();
        ...
        let jar_files = find_jar_files(project_path);
        ...
        for jar_path in jar_files {
            if let Ok(data) = std::fs::read(&jar_path) {
                ...
                // Copy-on-Load
                copy_jar_facts_to_local(conn, &hash)?;
    ...
    match res {
        ...
        Err(e) => {
            if is_db_locked_error(&e) {
                tracing::warn!("Global cache database is locked, returning Ok(()) silently: {:?}", e);
                Ok(())
            } else {
                Err(e)
            }
...
mod tests {
    ...
    fn test_tableswitch_valid() {
        // Valid tableswitch with low=1, high=2 (count = 2)
        // 1 byte opcode (170)
        // 3 bytes padding
        // 4 bytes default offset (all 0)
        // 4 bytes low (0, 0, 0, 1)
        // 4 bytes high (0, 0, 0, 2)
        // 8 bytes jump offsets (2 * 4 bytes)
    ...
    fn test_tableswitch_malformed_high_less_than_low() {
        ...
        // Should abort parsing method code safely
        let final_ip = run_bytecode_parser(&code);
        assert_eq!(final_ip, code.len());
    }
    ...
    fn test_tableswitch_overflow_add_one() {
        // tableswitch with low=0, high=i32::MAX
        let mut code = vec![170, 0, 0, 0];
        code.extend_from_slice(&[0, 0, 0, 0]); // default
    ...
    fn test_lookupswitch_valid() {
        // Valid lookupswitch with npairs = 2
        // 1 byte opcode (171)
        // 3 bytes padding
        // 4 bytes default offset
        // 4 bytes npairs (0, 0, 0, 2)
        // 16 bytes match-offset pairs (2 * 8 bytes)
