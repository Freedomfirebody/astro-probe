...
pub use astro_probe_core::query::{
    query_call_graph_internal, query_lineage_internal, CallGraphEdge, CallGraphResponse,
    LineageEdge, LineageResponse,
};
...
pub async fn query_call_graph(
    ...
    match query_call_graph_internal(&conn, &params.method, &params.direction) {
        Ok(resp) => Json(resp).into_response(),
        Err(e) => (
            StatusCode::INTERNAL_SERVER_ERROR,
            format!("Call Graph query failed: {}", e),
        )
            .into_response(),
    }
}
...
pub async fn query_routes(
    ...
    let mut sql =
        "SELECT http_method, path, controller_method_fqn FROM web_routes WHERE 1=1".to_string();
