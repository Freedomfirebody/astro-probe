...
impl FrameworkAnalyzer<Connection> for DependencyInjectionAnalyzer {
    ...
    fn analyze(&self, conn: &mut Connection) -> std::result::Result<(), Self::Error> {
        let tx = conn.transaction_with_behavior(rusqlite::TransactionBehavior::Immediate)?;
        {
            ...
            conn.execute(
                "DELETE FROM allocation_sites WHERE method_fqn = 'SpringDI'",
                [],
            )?;
            ...
            for (class_fqn, _) in &beans {
                ...
                let mut m_stmt = conn
                    .prepare("SELECT method_fqn FROM method_declarations WHERE class_fqn = ?1")?;
            ...
            for (owner_class, field_name) in autowired_fields {
                ...
                let field_type_ann: Option<String> = type_stmt
                    .query_row([&owner_class, &field_name], |r| r.get(0))
                    .ok();
                ...
                let qualifier_val: Option<String> = qual_stmt
                    .query_row([&owner_class, &field_name], |r| r.get(0))
                    .ok()
                ...
                for (bean_class, bean_name) in &beans {
                    let is_match = bean_class == &field_type
                        || get_transitive_parents(bean_class).contains(&field_type);
                    if is_match {
            ...
            for (method_fqn, params_str) in autowired_methods {
                ...
                for (i, p_name) in param_names.iter().enumerate() {
                    ...
                    let mut candidates = Vec::new();
                    for (bean_class, bean_name) in &beans {
                        let is_match = bean_class == p_type
                            || get_transitive_parents(bean_class).contains(p_type);
                        if is_match {
                            candidates.push((bean_class.clone(), bean_name.clone()));
                        }
                    }
                    ...
                    let qualifier_val: Option<String> = param_qual_stmt
                        .query_row([&method_fqn, p_name], |r| r.get(0))
                        .ok()
                        .map(|ann: String| ann["Qualifier:".len()..].to_string());

                    if let Some(ref q) = qualifier_val {
                        candidates
                            .retain(|(c, n)| n == q || c == q || c.ends_with(&format!(".{}", q)));
                    } else if candidates.len() > 1 {
                        // Match by name
